...
import numpy as np
from analytics.models import Salary_by_year
import pandas as pd


class Command(BaseCommand):
    def handle(self, *args, **options):
        data = Salary_by_year.objects.all().values('year', 'avg_salary')
        stats = self.get_salary_stats(data)
        names = ['средняя з/п', 'Уровень зарплат по годам']
    ...
    def get_salary_stats(self, data):
        ...
        avg_salary_by_year = df.set_index('year')['avg_salary'].to_dict()
